...
UPDATE users2 set created_at = STR_TO_DATE(created_at, "%d.%m.%Y %k:%i"), updated_at = STR_TO_DATE(updated_at, "%d.%m.%Y %k:%i");

ALTER table users2 MODIFY created_at datetime;
ALTER table users2 MODIFY updated_at datetime;
...
VALUES
 ...
 select * from storehouses_products o order by VALUE = 0, VALUE;
...
SELECT first_name , last_name FROM users u2 where u2.id in (SELECT p.user_id FROM profiles p where LOWER(MONTHNAME(birthday)) in ('may', 'august'));
...
SELECT FLOOR(AVG(DATEDIFF(DATE(NOW()), birthday)/365.25)) from profiles p ;
...
SELECT COUNT(*), DAYNAME(DATE(CONCAT(YEAR(NOW()), '-',MONTH(birthday), '-',DAY(birthday)))) as DAY_OF_WEEK FROM profiles p GROUP BY DAY_OF_WEEK;
